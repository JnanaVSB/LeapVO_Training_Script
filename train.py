...
Supports --backbone v3 for CoTracker v3 backbone comparison.
...
Usage:
    ...
    # v3 backbone
    python train.py --backbone v3 --run_name v3_bce_50k \\
        --pretrained_v3 ~/checkpoints/cotracker3_scaled_offline.pth
"""
...
# v3 backbone (optional — only needed with --backbone v3)
try:
    from leap_kernel_v3 import LeapKernelV3  # noqa: E402
    V3_AVAILABLE = True
except ImportError:
    V3_AVAILABLE = False
...
def build_leap_cfg(args):
    cfg = OmegaConf.create({
        "model": {
            "sliding_window_len": args.window_size,
            "hidden_dim":         256,
            "latent_dim":         128,
            "hidden_size":        384,
            "space_depth":    6,
    ...
    # v3-specific config (only read by LeapKernelV3, ignored by LeapKernel)
    if getattr(args, 'backbone', 'v1') == 'v3':
        cfg.model.v3 = OmegaConf.create({
            "space_depth":        3,
            "time_depth":         3,
            "hidden_size":        384,
            "num_heads":          8,
...
def compute_dyn_loss(probs, targets, valid_mask, loss_type, gce_q, trunc_frac):
    targets_exp = targets.unsqueeze(1).expand_as(probs)
    if not LOSSES_AVAILABLE:
...
def build_gt_for_window(coords_gt, vis_gt, is_dyn_gt,
    ...
    wi = int(wind_idx)
    c = coords_gt[:, :, sort_inds]
    v = vis_gt[:, :, sort_inds]
    d = is_dyn_gt[:, sort_inds]
    gt_coord = c[:, ind:ind + S_local, :wi].to(device)
    gt_vis   = v[:, ind:ind + S_local, :wi].to(device)
    gt_dyn   = d[:, :wi].to(device)
...
def compute_batch_loss(train_data, coords_gt, vis_gt, is_dyn_gt, args, device):
    ...
    for w in range(W):
        ...
        S_local  = coord_preds[w][0].shape[1]
        ...
        for k in range(len(coord_preds[w])):
            weight  = gamma ** (K - 1 - k)
            pred_c  = coord_preds[w][k]
            sigma_x = cov_preds[w][k][0]
            sigma_y = cov_preds[w][k][1]
...
def load_pretrained_fnet(model, ckpt_path):
    ...
    for k in our_fnet_keys:
        if k not in ckpt_subdict:
            missing.append(k)
            continue
        if tuple(our_sd[k].shape) != tuple(ckpt_subdict[k].shape):
            shape_errors.append((k, tuple(our_sd[k].shape), tuple(ckpt_subdict[k].shape)))
            continue
        loaded.append(k)
    ...
    logging.info(f"  fnet load: {len(loaded)} loaded, {len(missing)} missing, {len(shape_errors)} shape errors")

    if shape_errors:
        raise ValueError(f"{len(shape_errors)} shape mismatches — aborting")
    if loaded == 0 or len(loaded) < len(our_fnet_keys) * 0.5:
        raise ValueError(f"Loaded only {len(loaded)}/{len(our_fnet_keys)} fnet keys")
...
def load_pretrained_v3(model, ckpt_path):
    """Warm-start LeapKernelV3 from a pretrained CoTracker v3 checkpoint."""
    ckpt_path = Path(ckpt_path).expanduser()
    if not ckpt_path.exists():
        raise FileNotFoundError(f"V3 checkpoint not found: {ckpt_path}")

    logging.info(f"Loading pretrained v3 backbone from: {ckpt_path}")
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    ...
    v3_prefixes = ("fnet.", "updateformer.", "corr_mlp.", "time_emb")

    our_sd = model.state_dict()
    our_v3_keys = {k for k in our_sd.keys()
                   if any(k.startswith(p) for p in v3_prefixes)}

    loaded, missing, skipped, shape_errors = [], [], [], []

    for k in our_v3_keys:
        if k not in state_dict:
            missing.append(k)
            continue
        if tuple(our_sd[k].shape) != tuple(state_dict[k].shape):
            shape_errors.append((k, tuple(our_sd[k].shape), tuple(state_dict[k].shape)))
            continue
        loaded.append(k)

    for k in state_dict:
        if any(k.startswith(p) for p in v3_prefixes) and k not in our_v3_keys:
            skipped.append(k)

    load_dict = {k: state_dict[k] for k in loaded}
    model.load_state_dict(load_dict, strict=False)

    logging.info(f"  v3 load: {len(loaded)} loaded, {len(missing)} missing, "
                 f"{len(skipped)} skipped, {len(shape_errors)} shape errors")
    if shape_errors:
        for k, ours, ck in shape_errors[:5]:
            logging.warning(f"    shape mismatch: {k}: ours={ours} ckpt={ck}")
    if missing:
        for k in missing[:5]:
            logging.warning(f"    missing: {k}")

    leap_only = {k for k in our_sd.keys() if k not in our_v3_keys}
    logging.info(f"  LEAP-only keys (randomly initialized): {len(leap_only)}")
    logging.info(f"  v3 warm-start complete ({len(loaded)} keys loaded)")
    return {"loaded": loaded, "missing": missing,
            "skipped": skipped, "shape_errors": shape_errors}
...
def run_validation(model, val_loader, args, device, max_batches=None):
    model.eval()
    sums = {"main": 0.0, "vis": 0.0, "dyn": 0.0, "total": 0.0}
    n_ok = 0
    n_skip = 0
    cap = max_batches if max_batches is not None else len(val_loader)
        ...
        with torch.no_grad():
            for i, batch in enumerate(val_loader):
                ...
                try:
                    ...
                    vis_gt    = batch["visibility"].to(device)
                    dyn_gt    = batch["is_dynamic"].to(device)
                    queries   = batch["queries"].to(device)

                    _, _, _, _, _, train_data = model(
                        rgbs=rgbs, queries=queries,
                        iters=args.train_iters, is_train=True,
                    )
                    ...
                    loss, l_main, l_vis, l_dyn, _diag = compute_batch_loss(
                        train_data, coords_gt, vis_gt, dyn_gt, args, device
                    )

                    sums["main"]  += float(l_main.item())
                    sums["vis"]   += float(l_vis.item())
                    sums["dyn"]   += float(l_dyn.item())
    ...
    if n_ok == 0:
        return {"main": float("nan"), "vis": float("nan"),
                "dyn": float("nan"), "total": float("nan"),
                "n_batches": 0, "n_skip": n_skip}

    return {
        "main":  sums["main"]  / n_ok,
        "vis":   sums["vis"]   / n_ok,
        "dyn":   sums["dyn"]   / n_ok,
        "total": sums["total"] / n_ok,
        "n_batches": n_ok,
        "n_skip": n_skip,
    }
...
def train(args):
    ...
    if args.backbone == "v3":
        if not V3_AVAILABLE:
            raise ImportError(
                "LeapKernelV3 not found. Place leap_kernel_v3.py next to train.py "
                "or in the PYTHONPATH."
            )
        model = LeapKernelV3(cfg=cfg, stride=4).to(device)
        logging.info("Using LeapKernelV3 (CoTracker v3 backbone)")
    else:
        model = LeapKernel(cfg=cfg, stride=4).to(device)
        logging.info("Using LeapKernel (CoTracker v1 backbone)")

    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logging.info(f"Trainable parameters: {total_params:,}")

    # ---- Warm-start from pretrained checkpoint ----
    if args.backbone == "v3" and args.pretrained_v3:
        load_pretrained_v3(model, args.pretrained_v3)
    elif args.pretrained_fnet:
    ...
    if DATALOADER_AVAILABLE and not args.use_dummy:
        train_dataset = LeapKubricDataset(
            data_root=args.dataset_root, num_tracks=args.num_tracks,
        )
    ...
    val_loader = None
    if args.val_dataset_root and DATALOADER_AVAILABLE and not args.use_dummy:
        val_path = Path(args.val_dataset_root)
        if not val_path.exists():
            logging.warning(f"--val_dataset_root={val_path} does not exist. Skipping validation.")
        else:
            val_dataset = LeapKubricDataset(
                data_root=args.val_dataset_root, num_tracks=args.num_tracks,
            )
            ...
            cap = min(args.val_max_batches, len(val_loader))
            logging.info(f"Validation: every {args.val_every} steps, {cap} batches/pass")
    elif args.val_dataset_root and (not DATALOADER_AVAILABLE or args.use_dummy):
        logging.warning("Validation requested but DataLoader unavailable / dummy mode.")
    ...
    if resuming and csv_path.exists():
        ...
        logging.info(f"Appending to existing losses.csv (resume from step {start_step})")
    else:
    ...
    if val_loader is not None:
        ...
        if resuming and val_csv_path.exists():
    ...
    if resuming and cfg_path.exists():
        ...
        cfg_path_resume = run_dir / f"config_resume_{stamp}.json"
    ...
    logging.info(
        f"Starting: run={args.run_name} | backbone={args.backbone} | "
    ...
    while should_train:
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}"):
            ...
            if accum_count == args.grad_accum_steps:
                ...
                # ---- Validation ----
                if val_loader is not None and total_steps % args.val_every == 0:
                    val_t0 = time.time()
                    val = run_validation(
                    ...
                    writer.add_scalar("val/loss_total", val["total"], total_steps)
                    writer.add_scalar("val/loss_main",  val["main"],  total_steps)
                    writer.add_scalar("val/loss_vis",   val["vis"],   total_steps)
                    writer.add_scalar("val/loss_dyn",   val["dyn"],   total_steps)

                    if val_csv_writer is not None:
                        val_csv_writer.writerow([
                            total_steps,
                    ...
                    logging.info(
                        f"  VAL  | step {total_steps:6d} | "
                        f"total={val['total']:.4f}  main={val['main']:.4f}  "
                        f"vis={val['vis']:.4f}  dyn={val['dyn']:.4f}  "
                        f"({val['n_batches']} batches, {val_dt:.1f}s)"
                    )
...
def parse_args():
    ...
    # Backbone selection
    p.add_argument("--backbone", default="v1", choices=["v1", "v3"],
                   help="TAP backbone: v1 (CoTracker v1, paper default) or v3 (CoTracker v3)")
    p.add_argument("--pretrained_v3", type=str, default=None,
                   help="Path to CoTracker v3 checkpoint (.pth) to warm-start v3 backbone.")
    ...
    # Validation
    p.add_argument("--val_dataset_root", type=str, default=None)
...
if __name__ == "__main__":
    ...
    if args.pretrained_v3:
        args.pretrained_v3 = str(Path(args.pretrained_v3).expanduser())
    if args.val_dataset_root:
        args.val_dataset_root = str(Path(args.val_dataset_root).expanduser())
    train(args)
...
# def run_validation(model, val_loader, args, device, max_batches=None):
#     """
#     Run validation: forward + loss only, no backward, no optimizer step.

#     Returns dict with averaged losses over the val set:
#         {'main': float, 'vis': float, 'dyn': float, 'total': float, 'n_batches': int}

#     Skips batches that OOM or otherwise fail. Restores model.train() before
...
#                         is_train=True,   # need train_data dict; no grad anyway
#                     )
...
#                     else:
#                         n_skip += 1
#                         logging.warning(f"Val batch {i} failed: {e}")
#                         continue
#                 except Exception as e:
#                     n_skip += 1
#                     logging.warning(f"Val batch {i} failed: {e}")
...
#     finally:
#         # Always restore train mode and clear GPU
#         model.train()
#         torch.cuda.empty_cache()

#     if n_ok == 0:
#         logging.error("Validation: 0/0 batches succeeded — returning NaNs")
#         return {"main": float("nan"), "vis": float("nan"),
#                 "dyn": float("nan"), "total": float("nan"),
#                 "n_batches": 0, "n_skip": n_skip}
...
#         if not val_path.exists():
#             logging.warning(
#                 f"--val_dataset_root={val_path} does not exist. "
#                 f"Skipping validation."
#             )
#         else:
#             val_dataset = LeapKubricDataset(
#                 data_root=args.val_dataset_root,
#                 num_tracks=args.num_tracks,
#             )
#             logging.info(f"Validation dataset: {len(val_dataset)} sequences")
#             val_loader = DataLoader(
#                 val_dataset,
#                 batch_size=1,
#                 shuffle=False,        # deterministic — same seqs every val pass
...
#             cap = min(args.val_max_batches, len(val_loader))
#             logging.info(
#                 f"Validation: every {args.val_every} steps, "
#                 f"{cap} batches/pass (dataset has {len(val_loader)})"
#             )
#     elif args.val_dataset_root and (not DATALOADER_AVAILABLE or args.use_dummy):
#         logging.warning("Validation requested but DataLoader unavailable / dummy mode.")
...
#     # ---- Resume (must happen before opening log files, so we know whether
#     # to truncate or append them) ----
#     start_step = 0
#     resuming   = False
#     if args.resume:
#         start_step = load_checkpoint(model, optimizer, scheduler, run_dir)
...
#     # ---- TensorBoard ----
#     # SummaryWriter appends to its event file by default; safe to reuse log_dir.
#     log_dir = run_dir / "tb_logs"
#     writer  = SummaryWriter(log_dir=str(log_dir))
#     logging.info(f"TensorBoard → {log_dir}")

#     # ---- CSV loss log (for matplotlib plots without TensorBoard) ----
#     # On resume: append to the existing CSV so earlier-step rows are preserved.
#     # On a fresh run (or resume that found no checkpoint): truncate + write header.
#     csv_path = run_dir / "losses.csv"
#     if resuming and csv_path.exists():
#         csv_file = open(csv_path, "a", newline="")
#         csv_writer = csv.writer(csv_file)
...
#     # ---- Validation CSV (only if validation is enabled) ----
#     val_csv_file = None
#     val_csv_writer = None
#     if val_loader is not None:
#         val_csv_path = run_dir / "val_losses.csv"
...
#             val_csv_writer = csv.writer(val_csv_file)
#             logging.info(
#                 f"Appending to existing val_losses.csv (resume from step {start_step})"
#             )
#         else:
#             val_csv_file = open(val_csv_path, "w", newline="")
#             val_csv_writer = csv.writer(val_csv_file)
...
#     # On resume, write a timestamped sidecar so we don't overwrite the
#     # original config (useful if CLI args differ between launches).
#     cfg_path = run_dir / "config.json"
#     if resuming and cfg_path.exists():
#         from datetime import datetime
#         stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
#         with open(cfg_path_resume, "w") as f:
#             json.dump(vars(args), f, indent=2)
#         logging.info(f"Wrote resume config sidecar → {cfg_path_resume.name}")
#     else:
#         with open(cfg_path, "w") as f:
#             json.dump(vars(args), f, indent=2)
...
#     # Validation (optional — set --val_dataset_root to enable)
#     p.add_argument("--val_dataset_root", type=str, default=None,
#                    help="Path to validation dataset. If unset, no validation runs.")
#     p.add_argument("--val_every", type=int, default=2000,
#                    help="Run validation every N steps. Default matches save_every.")
#     p.add_argument("--val_max_batches", type=int, default=200,
#                    help="Cap number of val batches. Use len(val_set) if smaller.")
#     p.add_argument("--val_workers", type=int, default=2,
#                    help="DataLoader workers for validation set.")
...
# train.py — LEAP-VO Training Loop
# =================================
# Trains LeapKernel from scratch on TAP-Vid-Kubric (MOVI-F).
...
#          (vis_predictions, coord_predictions, dynamic_predictions,
#           cov_predictions, wind_inds, sort_inds)
#     5. vis_predictions[w]  = [B, S_local, N_w]   — ALREADY sigmoid-ed (line 493)
#     6. dyn_predictions[w]  = [B, S_local, N_w]   — ALREADY sigmoid-ed (line 494-495)
#     7. coord_predictions[w] = list[K] of [B, S_local, N_w, 2]  — pixel coords
#     8. cov_predictions[w]   = list[K] of [Sx, Sy], each [B, N_w, S_local, S_local]
#     9. wind_inds[w]          = scalar tensor (number of active tracks in window w)
#    10. sort_inds             = [N] — track permutation; MUST be applied to gt
#    11. MotionLabelBlock cfg needs: mode, in_dim, hidden_dim, S (from demo.yaml)
#    12. kernel_block composition in demo.yaml is "sum" (not "product" as I assumed)
#    13. anchor_aug needs: num_anchors, anchor_mode, margin, frame fields
...
#     # Truncated GCE on vis only
#     python train.py --loss_vis trunc_gce --loss_dyn bce --run_name trunc_vis

#     # Simulate larger batch via gradient accumulation (recommended: 4-8)
#     python train.py --grad_accum_steps 4 --run_name baseline_accum4
# """
...
#     Returns: (total, l_main, l_vis, l_dyn)  — all scalar tensors
#     """
#     vis_preds, coord_preds, dyn_preds, cov_preds, wind_inds, sort_inds = train_data
...
#             if LOSSES_AVAILABLE:
#                 l_k = cauchy_nll_loss(pred_c, gt_coord, sigma_x, sigma_y, valid_mask)
#             else:
#                 l_k = _stub_cauchy_nll(pred_c, gt_coord, sigma_x, sigma_y, valid_mask)
...
#     return total, total_main, total_vis, total_dyn
...
#                 loss, l_main, l_vis, l_dyn = compute_batch_loss(
#                     train_data, coords_gt, vis_gt, dyn_gt, args, device
#                 )
...
#                 if "out of memory" in str(e):
#                     logging.warning(
#                         f"OOM at step {total_steps} — skipping batch and clearing cache."
#                     )
#                     optimizer.zero_grad(set_to_none=True)
#                     accum_count = 0
#                     torch.cuda.empty_cache()
